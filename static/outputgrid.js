(function() {
  ...
  function logContent() {
    if(lastContent === ta2.value) {
      updateLogCursor();
      return;
    }
    removeChildNodes(charGrid);
    if(ta2.value.length == 0) {
      addCharElements('-','empty');
    } else {
      for(var i = 0; i < ta2.value.length; i++) {
        //
        var code = ta2.value.charCodeAt(i);
        var text = ta2.value.charAt(i);
        var slice = 4;
        // Test for SMP
        if(code >= 0xD800 && code < 0xDC00) {
          if(i < ta2.value.length) {
            var code2 = ta2.value.charCodeAt(i+1);
            if(code2 >= 0xDC00 && code < 0xE000) {
              code = (code - 0xD800) * 0x400 + (code2 - 0xDC00) + 0x10000;
              text += ta2.value.charAt(i+1);
              slice = 6;
              i++;
            }
          }
  ...
  function updateLogCursor() {
    ...
    selStart = ta2.selectionStart;
    selLength = ta2.selectionEnd - ta2.selectionStart;
    selDirection = ta2.selectionDirection;

    selLength = calculateLengthByCodepoint(ta2.value, selStart, selLength);
    selStart = calculateLengthByCodepoint(ta2.value, 0, selStart);
